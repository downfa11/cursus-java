...
public final class SagaContracts {
  ...
  public interface CommandOutboxStore { void enqueue(SagaCommand command) throws Exception; }
  public interface HistoryStore { void append(SagaHistoryEvent event) throws Exception; }
  public record Stores(InboxStore inbox, StateStore state, CommandOutboxStore commandOutbox, HistoryStore history) {}
  @FunctionalInterface public interface Operation<T> { T apply(Stores stores) throws Exception; }
  public interface Transaction { <T> T run(Operation<T> operation) throws Exception; }
}
