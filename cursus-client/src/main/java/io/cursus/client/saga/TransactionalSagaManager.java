...
public final class TransactionalSagaManager {
  ...
  public TransactionalSagaManager(
      SagaDefinition definition, Transaction transaction, SagaHistoryOptions options) {
    if (definition == null
        || definition.sagaType() == null
        || definition.sagaType().isBlank()
        || definition.handlers() == null
        || definition.handlers().isEmpty()) {
  ...
  private void handle(Stores stores, String key, SagaEventEnvelope event) throws Exception {
    ...
    for (int index = 0; index < commands.size(); index++) {
      ...
      String effectId =
          command.getEffectId().isEmpty() ? event.eventId() + ":" + index : command.getEffectId();
      ...
      command
          ...
          .correlationId(
              command.getCorrelationId().isEmpty()
                  ? state.getCorrelationId()
                  : command.getCorrelationId())
          .causationId(
              command.getCausationId().isEmpty() ? event.eventId() : command.getCausationId());
    ...
    if (SagaState.WAITING.equals(state.getStatus()))
      record(stores, state, event, RUN_WAITING, stepId, attempt, null, "");
  ...
  public void recordEffectResult(String key, String effectId, boolean succeeded, Exception error)
      throws Exception {
    if (succeeded && error != null)
      throw new IllegalArgumentException("successful result cannot include error");
    transaction.run(
        stores -> {
          SagaState state = state(stores, key);
          SagaState.EffectState effect = effect(state, effectId);
          if (!SagaState.EFFECT_SUCCEEDED.equals(effect.getStatus())
              && !SagaState.EFFECT_FAILED.equals(effect.getStatus())) {
            String failure =
                succeeded ? "" : (error == null ? "effect failed" : error.getMessage());
            effect.setStatus(succeeded ? SagaState.EFFECT_SUCCEEDED : SagaState.EFFECT_FAILED);
            effect.setLastError(failure);
            effect.setUpdatedAt(Instant.now());
            record(
                stores,
                state,
                emptyEvent(),
                succeeded ? COMMAND_SUCCEEDED : COMMAND_FAILED,
                effect.getStepId(),
                effect.getAttempts(),
  ...
  public SagaState startCompensation(String key, String stepId, Exception error) throws Exception {
    return transaction.run(
        stores -> {
          ...
          compensation.setStepId(stepId);
          compensation.setStatus(SagaState.COMPENSATING);
          compensation.setAttempts(compensation.getAttempts() + 1);
          compensation.setLastError(error == null ? "" : error.getMessage());
          compensation.setUpdatedAt(Instant.now());
          state.setCompensation(compensation);
          state.setStatus(SagaState.COMPENSATING);
          state.setOutcome("");
          record(
              stores,
              state,
  ...
  public SagaState startNewRun(String key) throws Exception {
    return transaction.run(
        stores -> {
          SagaState state = state(stores, key);
          if (!SagaState.COMPLETED.equals(state.getStatus())
              && !SagaState.FAILED.equals(state.getStatus())) {
            throw new IllegalStateException("cannot start a new run while Saga is not terminal");
          }
          state.setRunId(UUID.randomUUID().toString());
          state.setNextSequence(0);
          state.setStatus(SagaState.RUNNING);
          state.setOutcome("");
          state.setStepId("");
          state.setRetryCount(0);
          state.setLastError("");
          state.setEffects(new java.util.LinkedHashMap<>());
          state.setCompensation(null);
          state.setUpdatedAt(Instant.now());
          record(stores, state, emptyEvent(), RUN_STARTED, "", null, null, "");
          stores.state().save(state);
          return state;
        });
  }
  ...
  public void completeCompensation(String key) throws Exception {
    transaction.run(
        stores -> {
          SagaState state = state(stores, key);
          SagaState.CompensationState compensation = compensation(state);
          compensation.setStatus(SagaState.COMPLETED);
          compensation.setLastError("");
          compensation.setUpdatedAt(Instant.now());
          state.setStatus(SagaState.COMPLETED);
          state.setOutcome(OUTCOME_COMPENSATED);
          state.setUpdatedAt(Instant.now());
          record(
              stores,
              state,
  ...
  public void failCompensation(String key, Exception error) throws Exception {
    transaction.run(
        stores -> {
          SagaState state = state(stores, key);
          SagaState.CompensationState compensation = compensation(state);
          compensation.setStatus(SagaState.FAILED);
          compensation.setLastError(error.getMessage());
          compensation.setUpdatedAt(Instant.now());
          state.setStatus(SagaState.FAILED);
          state.setOutcome(OUTCOME_FAILED);
          state.setUpdatedAt(Instant.now());
          record(
              stores,
              ...
              null,
              error.getMessage());
          record(stores, state, emptyEvent(), RUN_FAILED, "", null, null, error.getMessage());
          stores.state().save(state);
          return null;
        });
  }
      ...
      transaction.run(
          stores -> {
            ...
            if (state != null) {
              ...
              state.setLastError(cause.getMessage());
              state.setUpdatedAt(Instant.now());
              record(
                  stores,
                  state,
            ...
            stores.inbox().fail(definition.sagaType(), event.eventId(), cause);
            return null;
          });
    } catch (Exception ignored) {
      /* preserve original handler failure */
    }
  }
  ...
  private void record(
    ...
    SagaHistoryEvent event =
        SagaHistoryEvent.builder()
            ...
            .sourceEventId(empty(source.eventId()))
            .correlationId(empty(source.correlationId()))
            .causationId(empty(source.causationId()))
            .sourceTopic(empty(source.sourceTopic()))
            .sourcePartition(source.sourcePartition())
            .sourceOffset(source.sourceOffset())
            .aggregateType(empty(source.aggregateType()))
            .aggregateId(empty(source.aggregateId()))
            .aggregateVersion(source.aggregateVersion())
            .payload(command == null ? "" : command.getPayload())
            .error(error)
            .build();
    stores.history().append(event);
  }

  private SagaState state(Stores stores, String key) throws Exception {
    SagaState state = stores.state().loadForUpdate(definition.sagaType(), key);
    if (state == null) throw new IllegalArgumentException("Saga state not found");
    return state;
  }

  private static SagaState.EffectState effect(SagaState state, String id) {
    SagaState.EffectState effect = state.getEffects().get(id);
    if (effect == null) throw new IllegalArgumentException("effect not found: " + id);
    return effect;
  }

  private static SagaState.CompensationState compensation(SagaState state) {
    if (state.getCompensation() == null)
      throw new IllegalArgumentException("compensation is not active");
    return state.getCompensation();
  }

  private static SagaCommand command(SagaState state, SagaState.EffectState effect) {
    return new SagaCommand(effect.getStepId(), "")
        .effectId(effect.getEffectId())
        .commandId(effect.getCommandId())
        .sagaType(state.getSagaType())
        .sagaId(state.getSagaId())
        .correlationId(state.getCorrelationId());
  }

  private static SagaEventEnvelope emptyEvent() {
    return new SagaEventEnvelope("", "", "", "");
  }

  private static String associationKey(SagaEventEnvelope event) {
    String key = empty(event.associationKey());
    if (!key.isEmpty()) return key;
    key = empty(event.correlationId());
    if (!key.isEmpty()) return key;
    key = empty(event.aggregateId());
    if (key.isEmpty()) throw new IllegalArgumentException("association key is required");
    return key;
  }
