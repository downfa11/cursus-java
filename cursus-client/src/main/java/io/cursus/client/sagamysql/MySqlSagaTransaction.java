...
public final class MySqlSagaTransaction implements SagaContracts.Transaction {
  ...
  public MySqlSagaTransaction(DataSource dataSource, String historyTopic) {
    delegate = new JdbcSagaTransaction(dataSource, historyTopic, JdbcSagaTransaction.Dialect.MYSQL);
  }

  @Override
  public <T> T run(SagaContracts.Operation<T> operation) throws Exception {
    return delegate.run(operation);
  }
}
